 ...
 * in kernel mode. The matrix driver serves up to MCU_MAX_LAMPS
 * lamps connecting to the chosen lighting device. The lamps are
 * enumerated from 0 to (MCU_MAX_LAMPS - 1).
 *
 ...
 *
 * A channels parameter indicates if the associated pixel value is
 * just a greyscale value (channels == 1) or a RGB value (channels ==3).
 *
 * Located after the header the data comes in a three-dimensional array
 * (pixel[row][column][channels]) with rows * colums * channels bytes length.
 *
 ...
 * but any value is possible (e.g. 2 for a dual-brightness display).
 *
 *
 ...
 *       device_channels     number of pixel values per port used
 ...
 * 
 * All modules use the same Shared Memory interface.
 *
 * monochrome ---
 *        lightbulb in MONOCHROME mode. It is either totally
 *        switched on or totally switched off.
 *
 * dim ---
 ...
 * ---------------------------
 *
 * In addition to CHANNELS adaptation, the RESOLUTION is also converted depending
...
int default_setup_channels = 0;
int default_setup_pixels = MCU_MAX_LAMPS;

int default_device_maxval = 255;
int default_device_channels = 1;
...
mcu_frame_check_size(mcu_frame_t * mcu_frame, int datagram_size)
{
	...
	required_size = sizeof(mcu_frame_header_t) + sizeof(u_char) *
						(mcu_frame->header.height * mcu_frame->header.width * mcu_frame->header.channels);
	if (required_size != datagram_size) {
...
mcu_frame_convert_byte_order(mcu_frame_t *mcu_frame)
{
	...
	mcu_frame->header.channels = ntohs(mcu_frame->header.channels);
...
mcu_frame_write_frame_data(mcu_frame_t * mcu_frame)
{
	...
	if (Device->flag_loaded == TRUE && device_loaded == FALSE) {
		fprintf(stderr, "mcud: device '%s' channels=%d  maxval=%d \n",
		      Device->name, Device->channels, Device->maxval );
		device_loaded = TRUE;
	}
	...
	if (Setup.header.height != mcu_frame->header.height
		...
		|| Setup.header.channels != mcu_frame->header.channels) {
		fprintf(stderr, "mcud: Invalid Frame Matrix (%dx%dx%d) for current configuration (%dx%dx%d)\n",
			mcu_frame->header.height, mcu_frame->header.width, mcu_frame->header.channels,
			Setup.header.height, Setup.header.width, Setup.header.channels);
		return;
	}
	...
	for (pixel = 0; pixel < Setup.header.pixels; pixel++) {
		position =  Setup.body.pixel[pixel].row    * mcu_frame->header.width * mcu_frame->header.channels +
					Setup.body.pixel[pixel].column * mcu_frame->header.channels;
		current_pixel = (u_char *) &mcu_frame->body.data + position;


		/*
		 *Adapt pixel value to device channels
		 */
		switch (Device->channels) {
			case 1:
				if (mcu_frame->header.channels == 3) {
			...
			case 3:
				if (mcu_frame->header.channels == 1) {
					device_pixel[0] = current_pixel[0];
			...
			default:
				fprintf(stderr, "mcud: invalid number of channels for device (%d)\n", Device->channels);
				break;
		}
		...
		if (mcu_frame->header.maxval != Device->maxval) {
			for (channel = 0;channel < Device->channels; channel++) {
                                device_pixel[channel] = ((device_pixel[channel] * (Device->maxval + 1)) / 
                                                         (mcu_frame->header.maxval + 1));
			}
		}
		...
		if (Device->flag_zero_off) {
			for (channel = 0;channel < Device->channels; channel++) {
				if (device_pixel[channel] == 0) {
					device_pixel[channel] = 1;
				}
			}
		}
	...
	if (monitor_mode) {
		...
		for (row = 0; row < mcu_frame->header.height; row++) {
			for (column = 0; column < mcu_frame->header.width; column++) {
				position =  row * mcu_frame->header.width * mcu_frame->header.channels +
					column * mcu_frame->header.channels;
				current_pixel = (u_char *) &mcu_frame->body.data + position;

				for (channel = 0;channel < mcu_frame->header.channels; channel++) {
					fprintf(stderr, "%02X", current_pixel[channel]);
				}
				fprintf(stderr, " ");
			}
			fprintf(stderr, "\n");
		...
		for (pixel = 0; pixel < Setup.header.pixels; pixel++) {
			for (channel = 0;channel < Device->channels; channel++) {
				fprintf(stderr, "%02X", Device->pixel[pixel][channel]);
			}
			fprintf(stderr, " ");
			if ( (pixel+1) % 8 == 0) {
				fprintf(stderr, "\n");
...
mcu_setup_convert_byte_order(mcu_setup_t *mcu_setup)
{
	...
	mcu_setup->header.channels = ntohs(mcu_setup->header.channels);
...
mcu_setup_set_config(mcu_setup_t *mcu_setup)
{
	...
	fprintf(stderr,"mcud: mcu_setup_set_config: received new setup (%dx%dx%d)\n",
		mcu_setup->header.height, mcu_setup->header.width, mcu_setup->header.channels);
	return 0;
}
...
setup_default_configuration(void)
{
	...
	Device->channels = default_device_channels;
	...
	Setup.header.channels = default_setup_channels;
...
main (int argc, char *argv[])
{
	...
	const char short_options[] = "f:?h:w:c:p:Vi:DM";
#ifdef LONG_OPTIONS
	...
	do {
		...
		switch (optchar) {
			...
			case 'w':
				...
				break;
			case 'c':
				default_setup_channels = atoi(optarg);
				break;
			case 'p':
...
#define SELECT_TIMEOUT 60
	...
	fprintf(stderr, "mcud: default setup (height=%d width=%d channels=%d)\n", Setup.header.height, Setup.header.width, Setup.header.channels );
...
//		fprintf(stderr,"mcud: received %d bytes, magic=0x%08lx\n", status, (long) ntohl(datagram.magic) );
		...
		switch(ntohl(datagram.magic)) {
			...
			case MAGIC_MCU_SETUP:
				...
				if (status == -1)
					break;
				if (datagram.mcu_setup.header.height * datagram.mcu_setup.header.width > MCU_MAX_LAMPS) {
					fprintf(stderr,"mcud: setup dimensions (%d/%d) exceed maximum matrix width (%d)\n",
						datagram.mcu_setup.header.height, datagram.mcu_setup.header.width, MCU_MAX_LAMPS);
				}
