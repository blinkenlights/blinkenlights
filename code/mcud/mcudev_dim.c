...
/*
static phase_t pixel_phase[MCU_MAX_LAMPS];                   // electric phase per pixel (0,1,2)
*/
...
typedef struct dim_interval {
	...
	unsigned char port_value[PIO_MAX_PORTS];          // data to write to ports at each interval
} dim_interval_t;
...
prepare_device_data (void)
{
	 ...
	 */
	for (pixel = 0; pixel < MCU_MAX_LAMPS; pixel++) {
...
//			rt_printk("time: %lld  phase: %lld\n", rt_get_time(), current_phase);
			...
			for(interval = 0;interval < DIM_INTERVALS;interval++) {
				 ...
				 */
				for(port = 0;port < PIO_MAX_PORTS;port++) {
					outb(dim_interval[interval].port_value[port], pio_port_addr[port]);
				}
			}
