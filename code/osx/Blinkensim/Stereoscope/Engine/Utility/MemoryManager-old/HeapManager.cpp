...
{
    _ASSERT (name != NULL);
    _ASSERT (strlen(name) < NAMELENGTH);
    strcpy (m_name, name);
    m_bytesAllocated = 0;
    m_bytesPeak      = 0;
    m_nInstances     = 0;
...
//            totalBytes, totalPeakBytes, totalInstances);
	...
	char strData[128];
	sprintf(strData,"%16s %9d %9d %5d  %9d %9d %5d", 
            m_name, 
            (int)m_bytesAllocated, (int)m_bytesPeak, (int)m_nInstances,
            (int)totalBytes, (int)totalPeakBytes, (int)totalInstances);
...
void Heap::Deallocate (void * pMem)
{
    AllocHeader * pHeader = (AllocHeader *)((char *)pMem -
                             sizeof(AllocHeader));
    assert (pHeader->nSignature == (int)MEMSYSTEM_SIGNATURE);
...
int Heap::ReportMemoryLeaks (int nBookmark1, int nBookmark2)
{
    ...
    while (pHeader != NULL) 
    {
        ...
        {
			char strData[128];
			sprintf(strData,"Leak in %s. Size: %d, address: 0x%0Xd", 
                     m_name, pHeader->nSize,(int)((char *)pHeader + sizeof(AllocHeader)));
