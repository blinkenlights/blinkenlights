...
#define SETUP_DEPTH_ATTRIBUTE  "depth"
...
config_start_element (void *data, const XML_Char *element, const XML_Char **attributes)
{
	...
	if (p_state->position == LEVEL_ROOT && strcmp(element, SETUP_ELEMENT) == 0) {
		...
		p_attribute = config_find_attribute ( SETUP_DEPTH_ATTRIBUTE, attributes, TRUE);
		if (p_attribute == NULL)
			return;
		p_state->p_mcu_setup->header.depth = atoi(p_attribute);
