...
mcu_frame_write_frame_data(mcu_frame_t * mcu_frame)
{
	...
	if (output_mode) {
		...
		fprintf(stderr, "*");
		count = mcu_frame->header.width * mcu_frame->header.height;
		status = fwrite(&mcu_frame->body.data, mcu_frame->header.depth, count, output_file);
		if (status != count) {
			fprintf(stderr, "mcud: fwrite returned %d, should be %d\n", status, count);
		}	
		fflush (output_file);
	}
...
main (int argc, char *argv[])
{
	...
	const char short_options[] = "c:?h:w:d:p:Vi:DMo:";
#ifdef LONG_OPTIONS
